...
默认 wifi 热点为`192.168.43.1`，可在模块设置中改为其他网段（如 `192.168.1.1`）。

支持自定义 `WIFI`、`USB`、`蓝牙`、`P2P`、`以太网` 的热点 IP（`Android 11`及以上），以及网段前缀。
...
## 关于热点 IP/网段自定义
模块内置配置界面（应用内打开 SoftApHelper），可自定义以下项，保存后重新开启热点生效：

- **WIFI / USB / 蓝牙 / P2P / 以太网** 各自的 IPv4（默认值见下表）
- **网段前缀** (8~31，默认 24)

配置通过 LSPosed 的 `XSharedPreferences` 读取，Android 11+ 各类型均生效；Android 9/10 固定 IP 分支仅使用 WIFI 配置（前缀不在该分支生效，由系统默认 /24 决定）。

> 网络前缀冲突（`isConflictPrefix`）时系统仍可能使用随机 IP 或导致连接失败，切勿填入与上游已占用网段相冲突的前缀。

| Type      | 默认 IP                        |
|-----------|--------------------------------|
| USB       | 192.168.42.1                   |
| WIFI      | 192.168.43.1（可自定义）        |
